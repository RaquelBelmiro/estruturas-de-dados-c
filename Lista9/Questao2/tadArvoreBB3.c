...
int qtdFolha(tipo_no_arvore sub_raiz){
     ...
     if (*sub_raiz !=NULL)
    {
        if(((*sub_raiz)->ptrEsquerda==NULL) && ((*sub_raiz)->ptrDireita==NULL))              //se tanto o nó da esquerda quanto da direita for null é folha
	       return 1+qtdFolha(&(*sub_raiz)->ptrEsquerda) + qtdFolha(&(*sub_raiz)->ptrDireita); //incrementa o retorno
     
        else 
           return qtdFolha(&(*sub_raiz)->ptrDireita) + qtdFolha(&(*sub_raiz)->ptrEsquerda); // se não o ciclo continua
    }
    else
        return 0;
}
...
void copiarArvore(tipo_no_arvore sub_raiz_original, tipo_no_arvore sub_raiz_copia){
     ...
     if(*sub_raiz_original!=NULL){                                           //Verifica se a árvore é diferente de NULL se for manda inserir a raiz
